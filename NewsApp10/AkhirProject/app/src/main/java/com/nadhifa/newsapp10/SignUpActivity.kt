...
import com.nadhifa.newsapp10.SignUpActivity.Companion.getLaunchService
import com.nadhifa.newsapp10.activity.MainActivity
import kotlinx.android.synthetic.main.activity_sign_up.*

class SignUpActivity : AppCompatActivity(), View.OnClickListener {
    private lateinit var mAuth: FirebaseAuth
    private lateinit var refUserId : DatabaseReference
    ...
    override fun onClick(p0: View) {
        when(v.id){
            R.id.ib_back_signup -> startActivity(Intent(SignInActivity.getLaucnhService(this)))
        ...
        if (fullName == ""){
            ...
            mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener { it ->
                if(it.isSuccessful){
                    ...
                    var refUsers = FirebaseDatabase.getInstance().reference.child(getString(R.string.text_user)).child(firebaseUserId)
